package com.unwood.player.base;
...
//第二步：根据上一步结果，组合每一步棋子所带来的所有结果（如某一步棋子可能形成我方1个活3，1个冲4（半活4）等），包括敌方和我方的；
//第三步：根据用户给的规则对上一步结果进行排序，并选子（有进攻形、防守形规则）
public class BaseComputerAi extends BasePlayer implements IPlayer{
	...
	private Point doFirstAnalysis(List<Point> comuters, List<Point> humans){
		...
		for (int i = 0; i < size; i++) {
			...
			if(x<currentRange.xStart || x>currentRange.xStop || y<currentRange.yStart || y>currentRange.yStop){//排除不在currentRange中的点
				continue;
			}
	...
	private Point doComputerSencondAnalysis(Map<Point,List<FirstAnalysisResult>> firstResults,List<SencondAnalysisResult> sencodResults) {
		...
		for (Point p : firstResults.keySet()){//映射中包含的键
			sr = new SencondAnalysisResult(p);
			list = firstResults.get(p);//返回指定键所映射的值
	...
	//在某个方向（八个方向中的一个）可下多少棋子，这个方法是第一分析中的核心方法
	...
	private int maxCountOnThisDirection(Point point,List<Point> enemyPoints,int direction,int count){
		int x=point.getX(),y=point.getY(); //保存x,y坐标
		switch (direction) {
		//横向
		case HENG:
